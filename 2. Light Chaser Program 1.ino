/*
  ...
  The LEDs will turn on and off sequentially with a delay of one second or 1000 milliseconds.
...
void loop() {
  ...
  delay(onDelay);                //wait for onDelay time      
  digitalWrite(blueLed, LOW);    //turn off bluedLed
  delay(offDelay);               //wait for offDelay time

  digitalWrite(whiteLed, HIGH);  //turn on whiteLed     
  delay(onDelay);                //wiat for onDelay time
