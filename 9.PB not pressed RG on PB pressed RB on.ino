/*
  ...
  When pushButton is not pressed, Red and Green turn on, yellow color appears.
  When pushButton is pressed, Red and Blue turn on, magenta collor appears.
  Red will always remain on.
...
void loop() {
  ...
  //pushButton is not pressed, Red and Green turn ON, yellow color appears
  digitalWrite(Green, buttonState);     //turn on Green led if pushButton is not pressed (HIGH)

  //pushButton is pressed, Red and Blue are ON, magenta color appears 
